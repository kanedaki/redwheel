...
// The State Machine
function TouchDaemon(options) {
  this.el = document.getElementsByClassName(options.el)[0] || document.getElementById(options.el) || document.body;; 
  this.cb = options.cb;
...
TouchDaemon.prototype = new FSM();

TouchDaemon.configConstants = configConstants;

//var TouchDaemon = assign({}, FSM.prototype, {
TouchDaemon.prototype.emit = function(event, e, params) {
    e = e? {srcElement: e.srcElement, target: e.currentTarget,
            originalEvent: e} : {};
    this.cb({event: event, e: e, params: params});
},
TouchDaemon.prototype.stop = function() {
},
TouchDaemon.prototype.setListeners = function() {
  this.el.addEventListener('touchstart', this.handlers.onTouchstart, true);
  this.el.addEventListener('touchend', this.handlers.onTouchend, true);
  this.el.addEventListener('touchmove', this.handlers.onTouchmove, true);
},
TouchDaemon.prototype.removeListeners = function() {
  this.el.removeEventListener('touchstart', this.handlers.onTouchstart, true);
  this.el.removeEventListener('touchend', this.handlers.onTouchend, true);
  this.el.removeEventListener('touchmove', this.handlers.onTouchmove, true);
},
TouchDaemon.prototype.initialState = '@waiting',
TouchDaemon.prototype.onStateChange = function() {
  window.clearTimeout(this.timerId);
  this.timerId = window.setTimeout(this.handlers.onTimeout, this.timeout);
},
TouchDaemon.prototype.handlers = {
  ...
  onTouchmove: function(e) {
    if (this._refDistance > TouchDaemon.configConstants.dragThreshold) {
...
TouchDaemon.prototype.transitions = {
  '@waiting': {
